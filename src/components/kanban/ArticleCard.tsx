...
export default function ArticleCard ({
  ...
  return (
    <div
      ref={drag as any}
      className={`p-4 border-b border-gray-100 w-full ${isDragging
        ? 'opacity-50'
        : ''} ${cardBgColor}`}
      style={{ cursor: 'move' }}
    >
      ...
      <div className="flex items-center text-xs text-gray-500 mb-2">
        <span className="mr-1">Due {dueDate ?
          `${getMonthAbbreviation(
            new Date(dueDate as string).getMonth())} ${new Date(
            dueDate as string).getDate()}`
          : 'No date'}</span>
        ...
        {(article.WordCount || article['Word Count']) && (
          <>
            <span className="text-xs text-gray-400 mr-1">•</span>
            <span className="text-xs text-gray-500">{article.WordCount ||
              article['Word Count']} words</span>
          </>
        )}
      </div>
      ...
      <div className="flex flex-col gap-y-1 mb-3">
        ...
        <div className="flex items-center">
          ...
          <span className="text-xs text-gray-700 truncate">
            {typeof article.Client === 'string'
              ? article.Client
              : 'Example Client'}
          </span>
        </div>

        {article['Content Optimization Score'] !== undefined &&
          article['Content Optimization Score'] > 0 && (
            <div className="flex items-center">
              <span
                className="text-xs text-gray-500 mr-1">Optimization Score:</span>
              <span className="text-xs text-gray-700 truncate">
