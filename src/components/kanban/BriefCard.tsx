...
export default function BriefCard ({
  ...
  return (
    <div
      ref={drag as any}
      className={`p-5 border-b border-gray-100 w-full ${isDragging
        ? 'opacity-50'
        : ''} ${cardBgColor}`}
      style={{ cursor: 'move' }}
    >
      ...
      {isOverdue && (
        <div className="mb-2">
          <span
            className="inline-block px-2 py-0.5 text-xs font-medium bg-red-100 text-red-800 rounded">Overdue</span>
        </div>
      )}
      ...
      <div className="flex items-center text-xs text-gray-500 mb-2">
        <span className="mr-1">Due {brief.DueDate
      ...
      <div className="grid grid-cols-2 gap-x-2 gap-y-1 mb-3">
        <div className="flex items-center">
          ...
          <span className="text-xs text-gray-700 truncate">{getDisplayName(
            brief.ContentWriter)}</span>
        </div>

        <div className="flex items-center">
          <span className="text-xs text-gray-500 mr-1">Editor:</span>
          <span className="text-xs text-gray-700 truncate">{getDisplayName(
            brief.ContentEditor)}</span>
        </div>

        <div className="flex items-center col-span-2">
          <span className="text-xs text-gray-500 mr-1">Client:</span>
          <span
            className="text-xs text-gray-700 truncate">{typeof brief.Client ===
          'string' ? brief.Client : 'Example Client'}</span>
        </div>
      </div>
      ...
      {brief['WordCountTarget'] && brief['WordCountTarget'] > 0 && (
        <div className="mt-2">
          <div className="text-xs text-gray-500">Word Count Target: <span
            className="text-gray-700">{brief['WordCountTarget']}</span></div>
        </div>
      )}
    </div>
  )
