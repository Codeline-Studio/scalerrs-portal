...
if (apiKey) {
  ...
  if (!apiKey.startsWith('pat')) {
    console.warn(
      'Warning: API key does not appear to be a personal access token (should start with "pat")')
  }
}
...
if (hasAirtableCredentials) {
  ...
  if (isBrowser) {
    console.log(
      'Running in browser environment. Using client-side Airtable initialization.')
    ...
    if (publicApiKey && publicBaseId) {
      ...
      console.warn(
        `Public API Key exists: ${!!publicApiKey}, Public Base ID exists: ${!!publicBaseId}`)
    }
  } else {
  ...
  console.warn(
    `Airtable API key or Base ID not found. Using mock data for build process.`)
...
export async function addComment (
  ...
  try {
    ...
    const taskRecords = await base(TABLES.TASKS).select({
...
export async function getCommentsByTask (taskId: string) {
  ...
  try {
    ...
    if (allComments.length > 0) {
      ...
      console.log('Available fields in first comment:',
        Object.keys(allComments[0].fields))
    }
    ...
    const records = await base(TABLES.COMMENTS).select({
    ...
    if (records.length === 0) {
      console.log(
        'No comments found with first approach, trying alternative...')
      ...
      console.log(
        `Found ${allTaskComments.length} comments with manual filtering`)
      ...
      if (allTaskComments.length > 0) {
        return allTaskComments.map((record: any) => ({
          ...
          Task: Array.isArray(record.fields.Task)
            ? record.fields.Task
            : [record.fields.Task || ''],
          User: Array.isArray(record.fields.User)
            ? record.fields.User
            : [record.fields.User || ''],
    ...
    console.log(
      `Found ${records.length} comments in Airtable for task ${taskId}`)
    ...
    const comments = records.map((record: any) => {
      ...
      return {
        ...
        CreatedAt: fields.CreatedAt || fields['Created Time'] ||
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        if (listError.message && listError.message.includes(
          'Table "__nonexistent_table__" not found. Available tables:')) {
          const availableTables = listError.message.split(
            'Available tables:')[1].trim().
            split(',').
            map((t: string) => t.trim())
          ...
          if (!briefsExists) {
            console.log(
              'Briefs table does not exist in this base. Using mock data.')
            return mockBriefs
          }
        }
      }
    ...
    try {
      ...
      const checkRecord = await base(TABLES.BRIEFS).
        select({ maxRecords: 1 }).
        firstPage()
      console.log('Briefs table check:', checkRecord.length > 0
        ? 'Table exists and has records'
        : 'Table exists but is empty')

      if (checkRecord.length > 0) {
        console.log('Sample Briefs record structure:', checkRecord[0].fields)
        console.log('Available fields in Briefs:',
          Object.keys(checkRecord[0].fields))
      } else {
      ...
      if (checkError.message &&
        checkError.message.includes('Table not found')) {
      ...
      if (checkError.message && checkError.message.includes('authorized')) {
        console.error(
          'Authorization error. Your token may not have the correct permissions.')
        console.error(
          'Make sure your token has the following scopes: data.records:read, data.records:write, schema.bases:read')
        return mockBriefs
      }
...
export async function updateBriefStatus (briefId: string, status: string) {
  if (!hasAirtableCredentials) {
    console.log('Using mock data for updating brief status (no credentials):',
      briefId, status)
...
export async function getArticles () {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        if (listError.message && listError.message.includes(
          ...
          if (!articlesExists) {
            console.log(
              'Articles table does not exist in this base. Using mock data.')
            return mockArticles
          }
        }
      }
    ...
    console.log(
      `Successfully fetched ${records.length} articles records from Airtable`)

    // Log the first record to see what fields are available
    if (records.length > 0) {
      console.log('First article record fields:', records[0].fields)
      console.log('First article record field keys:',
        Object.keys(records[0].fields))
    }
    ...
    return records.map((record: any) => {
      ...
      let writerValue = fields.Writer || fields['Writer'] ||
      ...
      if (typeof writerValue === 'string' && writerValue.startsWith('rec') &&
        writerValue.length === 17) {
      ...
      let editorValue = fields.Editor || fields['Editor'] ||
      ...
      if (typeof editorValue === 'string' && editorValue.startsWith('rec') &&
        editorValue.length === 17) {
        // This is likely a record ID, use a hardcoded name based on the mock data
...
export async function updateArticleStatus (articleId: string, status: string) {
  if (!hasAirtableCredentials) {
    console.log('Using mock data for updating article status (no credentials):',
      articleId, status)
    const updatedArticle = mockArticles.find(
  ...
  try {
    console.log(
      `Updating article ${articleId} status to ${status} in Airtable...`)
    console.log('Using base ID:', baseId)
    console.log('Using table name:', TABLES.ARTICLES)
...
export async function getBacklinks () {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        if (listError.message && listError.message.includes(
          ...
          if (!backlinksExists) {
            console.log(
              'Backlinks table does not exist in this base. Using mock data.')
            return mockBacklinks
          }
        }
      }
    ...
    try {
      // First, try to get just one record to verify the table exists and is accessible
      const checkRecord = await base(TABLES.BACKLINKS).
        select({ maxRecords: 1 }).
        firstPage()
      console.log('Backlinks table check:', checkRecord.length > 0
        ? 'Table exists and has records'
        : 'Table exists but is empty')

      if (checkRecord.length > 0) {
        console.log('Sample Backlinks record structure:',
          checkRecord[0].fields)

        // Map the fields from your Airtable schema to our expected format
        const sampleFields = checkRecord[0].fields
        console.log('Available fields in Backlinks:',
          Object.keys(sampleFields))

        // Log all available fields to understand what's in the Airtable
        console.log('Available fields in Backlinks table:',
          Object.keys(sampleFields))
    ...
    console.log(
      `Successfully fetched ${records.length} backlinks records from Airtable`)

    // Log the first record to see what fields are available
    if (records.length > 0) {
      console.log('First backlink record fields:', records[0].fields)
      console.log('First backlink record field keys:',
        Object.keys(records[0].fields))
    }
...
export async function updateBacklinkStatus (
  ...
  if (!hasAirtableCredentials) {
    console.log('Using mock data for updating backlink status:', backlinkId,
      status)
    const updatedBacklink = mockBacklinks.find(
...
export async function getKPIMetrics () {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        if (listError.message && listError.message.includes(
          ...
          // Check if our required tables exist
          const kpiMetricsExists = availableTables.some(
            (t: string) => t === TABLES.KPI_METRICS || t ===
              TABLES.KPI_METRICS.toLowerCase() || t === 'KPI Metrics' || t ===
              'kpi metrics')
          const urlPerformanceExists = availableTables.some(
            (t: string) => t === TABLES.URL_PERFORMANCE || t ===
              TABLES.URL_PERFORMANCE.toLowerCase() || t === 'URL Performance' ||
              t === 'url performance')
          const keywordPerformanceExists = availableTables.some(
            (t: string) => t === TABLES.KEYWORD_PERFORMANCE || t ===
              TABLES.KEYWORD_PERFORMANCE.toLowerCase() || t ===
              'Keyword Performance' || t === 'keyword performance')

          console.log(`KPI Metrics table exists: ${kpiMetricsExists}`)
          console.log(`URL Performance table exists: ${urlPerformanceExists}`)
          console.log(
            `Keyword Performance table exists: ${keywordPerformanceExists}`)

          // If tables don't exist, use mock data
          if (!kpiMetricsExists) {
            console.log(
              'KPI Metrics table does not exist in this base. Using mock data.')
            return mockKPIMetrics
          }
        }
      }
    ...
    try {
      // First, try to get just one record to verify the table exists and is accessible
      const checkRecord = await base(TABLES.KPI_METRICS).
        select({ maxRecords: 1 }).
        firstPage()
      console.log('KPI Metrics table check:', checkRecord.length > 0
        ? 'Table exists and has records'
        : 'Table exists but is empty')

      if (checkRecord.length > 0) {
        console.log('Sample KPI Metrics record structure:',
          checkRecord[0].fields)

        // Map the fields from your Airtable schema to our expected format
        const sampleFields = checkRecord[0].fields
        console.log('Available fields in KPI Metrics:',
          Object.keys(sampleFields))

        // Check if we have the necessary fields or their equivalents
        const hasMetricName = 'Metric Name' in sampleFields || 'MetricName' in
          sampleFields
        const hasCurrentValue = 'Current Value' in sampleFields ||
          'CurrentValue' in sampleFields
        const hasTargetValue = 'Target Value' in sampleFields ||
          'TargetValue' in sampleFields || 'Goal' in sampleFields
        const hasDelta = 'Delta/Change' in sampleFields || 'ChangePercentage' in
          sampleFields
        ...
        if (!hasMetricName || !hasCurrentValue) {
          console.warn(
            'KPI Metrics table is missing required fields. Using mock data.')
          return mockKPIMetrics
        }
      } else {
    ...
    console.log(
      `Successfully fetched ${records.length} KPI metrics records from Airtable`)
    ...
    return records.map((record: any) => {
      ...
      return {
        ...
        ChangePercentage: fields['Delta/Change'] ||
          fields['ChangePercentage'] || 0,
        Goal: fields['Target Value'] || fields['TargetValue'] ||
          fields['Goal'] || 0,
        Date: fields['KPI Timestamp'] || fields['Date'] ||
          new Date().toISOString(),
        Unit: fields['Unit'] || '',
...
export async function getURLPerformance () {
  ...
  try {
    ...
    try {
      // First, try to get just one record to verify the table exists and is accessible
      const checkRecord = await base(TABLES.URL_PERFORMANCE).
        select({ maxRecords: 1 }).
        firstPage()
      console.log('URL Performance table check:', checkRecord.length > 0
        ? 'Table exists and has records'
        : 'Table exists but is empty')

      if (checkRecord.length > 0) {
        console.log('Sample URL Performance record structure:',
          checkRecord[0].fields)

        // Map the fields from your Airtable schema to our expected format
        const sampleFields = checkRecord[0].fields
        console.log('Available fields in URL Performance:',
          Object.keys(sampleFields))

        // Check if we have the necessary fields or their equivalents
        const hasURLPath = 'URL Path' in sampleFields || 'URLPath' in
          sampleFields
        const hasClicks = 'Clicks Last Month' in sampleFields || 'Clicks' in
          sampleFields || 'Traffic' in sampleFields
        const hasImpressions = 'Impressions' in sampleFields || 'Traffic' in
          sampleFields
        const hasPageType = 'Page Type (Main)' in sampleFields || 'Page Type' in
          sampleFields || 'PageType' in sampleFields
        ...
        if (!hasURLPath) {
          console.warn(
            'URL Performance table is missing required fields. Using mock data.')
          return mockURLPerformance
        }
      } else {
      ...
      console.error('Error checking URL Performance table:',
        checkError.message)
      if (checkError.message.includes('Table not found')) {
    ...
    console.log(
      `Successfully fetched ${records.length} URL performance records from Airtable`)
    ...
    return records.map((record: any) => {
      ...
      return {
        ...
        PageType: fields['Page Type (Main)'] || fields['Page Type'] ||
          fields['PageType'] || '',
        Clicks: fields['Clicks Last Month'] || fields['Clicks'] ||
          fields['Traffic'] || 0,
        Impressions: fields['Impressions'] || fields['Traffic'] || 0,
        CTR: fields['CTR'] || 0,
        AveragePosition: fields['Current Rank'] || fields['AveragePosition'] ||
          0,
        // Include all original fields as well
        ...fields,
      }
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        checkRecord = await base(tableName).
          select({ maxRecords: 1 }).
          firstPage()
        tableFound = true
        ...
        for (const altName of ALT_TABLES.KEYWORD_PERFORMANCE) {
          try {
            ...
            checkRecord = await base(altName).
              select({ maxRecords: 1 }).
              firstPage()
      ...
      if (!tableFound) {
        console.log(
          'Could not find Keyword Performance table with any known names. Trying to list all tables...')
        try {
          // This is a workaround to list tables - we try to access a non-existent table
          // which will fail, but the error message will contain the list of valid tables
          await base('__nonexistent_table__').select().firstPage()
          ...
          if (listError.message && listError.message.includes(
            ...
            for (const availableTable of availableTables) {
              try {
                ...
                checkRecord = await base(availableTable).
                  select({ maxRecords: 1 }).
                  firstPage()
                ...
                if (checkRecord && checkRecord.length > 0) {
                  ...
                  if ('Keyword' in fields || 'keyword' in fields ||
                    'Keywords' in fields) {
                    console.log('Found potential Keyword Performance table:',
                      availableTable)
                ...
                console.log(`Error checking table ${availableTable}:`,
                  tableCheckError)
              }
            }
          }
      ...
      console.log('Keyword Performance table check:', checkRecord.length > 0
        ? 'Table exists and has records'
        : 'Table exists but is empty')

      if (checkRecord.length > 0) {
        console.log('Sample Keyword Performance record structure:',
          checkRecord[0].fields)

        // Map the fields from your Airtable schema to our expected format
        const sampleFields = checkRecord[0].fields
        console.log('Available fields in Keyword Performance:',
          Object.keys(sampleFields))

        // Check if we have the necessary fields or their equivalents
        const hasKeyword = 'Keyword' in sampleFields || 'keyword' in
          sampleFields || 'Keywords' in sampleFields
        const hasVolume = 'Search Volume' in sampleFields || 'Volume' in
          sampleFields || 'search_volume' in sampleFields
        const hasDifficulty = 'Difficulty' in sampleFields || 'difficulty' in
          sampleFields || 'Keyword Difficulty' in sampleFields
        const hasCurrentPosition = 'Current Rank' in sampleFields ||
          'Current Position' in sampleFields || 'CurrentPosition' in
          sampleFields || 'current_position' in sampleFields || 'Position' in
          sampleFields
        const hasPositionChange = 'Position Change' in sampleFields ||
          'PositionChange' in sampleFields || 'position_change' in sampleFields
        ...
        if (!hasKeyword) {
          console.warn(
            'Keyword Performance table is missing required fields. Using mock data.')
          return mockKeywordPerformance
        }
      ...
      console.error('Error checking Keyword Performance table:',
        checkError.message)
      if (checkError.message.includes('Table not found')) {
        console.error(
          'The Keyword Performance table does not exist in this base')
        return mockKeywordPerformance
      }
    ...
    console.log(
      `Successfully fetched ${records.length} keyword performance records from Airtable`)
    ...
    return records.map((record: any) => {
      ...
      return {
        ...
        CurrentPosition: fields['Current Rank'] || fields['Current Position'] ||
          fields['CurrentPosition'] || 0,
        PreviousPosition: fields['Starting Keyword Position'] ||
          fields['Previous Position'] || fields['PreviousPosition'] || 0,
        PositionChange: fields['Position Change'] || fields['PositionChange'] ||
          0,
