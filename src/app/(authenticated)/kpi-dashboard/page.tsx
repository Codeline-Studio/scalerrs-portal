...
function KpiDashboard () {
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          ...
          console.log('KPI metrics fetched successfully:', kpiMetrics.length,
            'records')

          // Check if we got mock data
          const isMockData = kpiMetrics.some(
            (metric: any) => metric.id && metric.id.startsWith('kpi'))
          if (isMockData) {
          ...
          errorMessages.push(
            `KPI metrics: ${kpiErr.message || 'Unknown error'}`)
          hasErrors = true
        ...
        try {
          ...
          console.log('URL performance fetched successfully:',
            urlPerformance.length, 'records')
          setUrlPerformanceData(urlPerformance)
        } catch (urlErr: any) {
          console.error('Error fetching URL performance:', urlErr)
          errorMessages.push(
            `URL performance: ${urlErr.message || 'Unknown error'}`)
          hasErrors = true
          // Use mock data as fallback
        ...
        const processedData = processAirtableData(kpiMetrics, urlPerformance,
          keywordPerformance, monthlyProjections)
        setProcessedData(processedData)

        // Set error message if any of the fetches failed
        if (hasErrors) {
          // Check if all errors are authorization errors
          const allAuthErrors = errorMessages.every(
            msg => msg.includes('authorized') || msg.includes('403'))

          if (allAuthErrors) {
            setError(
              `Authorization error: Your Airtable token doesn't have the necessary permissions. Using sample data as fallback.`)
        ...
        setError(`An error occurred while fetching KPI data: ${err.message ||
        'Unknown error'}`)
  ...
  const processAirtableData = (
    kpiMetrics: any[], urlPerformance: any[], keywordPerformance: any[],
    ...
    if (kpiMetrics.length > 0) {
      ...
      kpiMetrics.forEach((metric: any) => {
        ...
        if (cardIndex !== -1) {
          ...
          const currentValue = metric.CurrentValue || metric['Current Value'] ||
            0
          const targetValue = metric.Goal || metric['Target Value'] ||
            metric.TargetValue || 0
          const previousValue = metric.PreviousValue ||
            metric['Previous Value'] || 0
          const changePercentage = metric.ChangePercentage ||
            metric['Delta/Change'] || 0
          const unit = metric.Unit || ''

          kpiCards[cardIndex].current = currentValue ||
            kpiCards[cardIndex].current
          kpiCards[cardIndex].target = targetValue ||
            kpiCards[cardIndex].target
          ...
          if (changePercentage) {
            kpiCards[cardIndex].delta = parseFloat(
              changePercentage.toString().replace('%', '')).toFixed(1)
            kpiCards[cardIndex].trend = parseFloat(changePercentage) >= 0
              ? 'up'
              : 'down'
          }
          ...
          console.log(`Updated KPI card for ${cardTitle}:`,
            kpiCards[cardIndex])
        } else {
      ...
      const totalCurrent = kpiCards.reduce(
        (sum: number, card: any) => sum + (card.current / card.target), 0)
      const progressPercentage = Math.round(
        (totalCurrent / kpiCards.length) * 100)

      processedData.summary.currentProgress = progressPercentage
      processedData.summary.status = progressPercentage >= 80
        ? 'success'
        : progressPercentage >= 60 ? 'warning' : 'danger'
    ...
    if (urlPerformance.length > 0) {
      ...
      const blogPages = urlPerformance.filter(
        (url: any) => url.PageType === 'Blog' ||
          url.URLPath?.includes('/blog'))
      const featurePages = urlPerformance.filter(
        (url: any) => url.PageType === 'Feature' ||
          url.URLPath?.includes('/features'))
      const solutionPages = urlPerformance.filter(
        (url: any) => url.PageType === 'Solution' ||
          url.URLPath?.includes('/solutions'))
      const highIntentPages = urlPerformance.filter(
        (url: any) => url.PageType === 'High-Intent' ||
          url.URLPath?.includes('/demo') || url.URLPath?.includes('/contact'))

      // Update page type overview with real data
      if (blogPages.length > 0) {
        const totalClicks = blogPages.reduce(
          (sum: number, page: any) => sum + (page.Clicks || 0), 0)
        processedData.pageTypeBreakdown.overview.blog.clicks = totalClicks
      }

      if (featurePages.length > 0) {
        const totalClicks = featurePages.reduce(
          (sum: number, page: any) => sum + (page.Clicks || 0), 0)
        processedData.pageTypeBreakdown.overview.feature.clicks = totalClicks
      }

      if (solutionPages.length > 0) {
        const totalTraffic = solutionPages.reduce(
          (sum: number, page: any) => sum + (page.Impressions || 0), 0)
        processedData.pageTypeBreakdown.overview.solution.traffic = totalTraffic
      }

      if (highIntentPages.length > 0) {
        const totalTraffic = highIntentPages.reduce(
          (sum: number, page: any) => sum + (page.Impressions || 0), 0)
        processedData.pageTypeBreakdown.overview.highIntent.traffic = totalTraffic
      }

      // Find top and bottom performers
      const sortedByClicks = [...urlPerformance].sort(
        (a, b) => (b.Clicks || 0) - (a.Clicks || 0))
      const sortedByPosition = [...urlPerformance].sort(
        (a, b) => (a.AveragePosition || 100) - (b.AveragePosition || 100))
      ...
      if (sortedByClicks.length > 0) {
        ...
        if (sortedByClicks[sortedByClicks.length - 1]) {
          processedData.pageTypeBreakdown.performers.bottom[0] = {
            url: sortedByClicks[sortedByClicks.length - 1].URLPath ||
              '/unknown',
            metric: 'Traffic',
    ...
    if (keywordPerformance.length > 0) {
      ...
      const positionChanges = keywordPerformance.filter(
        (kw: any) => kw.PositionChange).map((kw: any) => kw.PositionChange)

      if (positionChanges.length > 0) {
        const avgPositionChange = positionChanges.reduce(
            (sum: number, change: number) => sum + change, 0) /
        ...
        if (avgPositionChange > 0) {
          ...
          yearlyData.projectedTraffic = Math.round(
            yearlyData.projectedTraffic * (1 + (avgPositionChange / 100)))
        }
      }
    }
    ...
    if (monthlyProjections.length > 0) {
      ...
      if (mappedProjections.length > 0) {
        ...
        if (mappedProjections.length >= 12) {
          ...
          yearlyProjectionData.splice(0, mappedProjections.length,
            ...mappedProjections)
        }
      }
    }
    ...
    <DashboardLayout>
      <div className="mb-6">
        ...
        <p className="text-mediumGray">Visualise performance against strategic
          targets and identify potential shortfalls early</p>
      </div>
      ...
      {loading ? (
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <p className="mt-2 text-sm">
                The dashboard is currently displaying sample data. To fix this
                issue:
              </p>
              <ul className="list-disc list-inside mt-1 text-sm">
                <li>Check that your Airtable base has the required tables (KPI
                  Metrics, URL Performance, Keyword Performance)
                </li>
                <li>Verify that your Airtable personal access token has the
                  correct scopes: data.records:read, data.records:write,
                  schema.bases:read
                </li>
                <li>Ensure that the tables have the correct field names as
              ...
              <div
                ...
                <p className="text-sm font-medium text-yellow-800">
                  Note: You can continue using the dashboard with sample data.
                  All functionality will work, but the data will not reflect
                  your actual metrics.
                </p>
              </div>
            </div>
        ...
        <>
          ...
          {usingMockData && !error && (
            <div
              className="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded relative mb-6"
              role="alert">
              <div className="flex items-start">
                <div className="mr-2">
                  <svg className="h-6 w-6 text-blue-500" fill="none"
                       viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round"
                          strokeWidth="2"
                ...
                <div>
                  ...
                  <p className="mt-2 text-sm">
                    To connect your real data, please create the following
                    tables in your Airtable base:
                  </p>
                  <ul className="list-disc list-inside mt-1 text-sm">
                    <li>KPI Metrics - for tracking key performance indicators
                    </li>
                  ...
                  <p className="mt-2 text-sm">
                    ...
                    in <code>docs/airtable-schema.md</code> for the required
                    fields.
                  </p>
                </div>
              </div>
          ...
          {/* Header Overview */}
          <div
            className={`p-6 rounded-lg mb-6 ${processedData.summary.status ===
            'success' ? 'bg-green-100' : processedData.summary.status ===
            ...
            <div className="flex items-start">
              ...
              <div>
                <h3 className="text-xl font-bold text-dark mb-2">Performance
                  Overview</h3>
                <p className="text-mediumGray mb-1">You're currently
                  hitting {processedData.summary.currentProgress}% of your Q2
                  goal.</p>
                <p className="text-mediumGray">Based on current output, you'll
                  reach ~{processedData.summary.annualProjection}% of the annual
                  target.</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="inline-flex rounded-md shadow-sm">
                <Button
                  ...
                  className={`px-4 py-2 text-sm font-medium ${timeFrameFilter ===
                  '3months'
                    ? 'bg-primary text-white'
                    : 'bg-white text-gray-700 hover:bg-gray-50'} rounded-l-md border border-gray-300`}
                >
                  3 Months
                </Button>
                <Button
                ...
                <Button
                  ...
                  '12months'
                    ? 'bg-primary text-white'
                    : 'bg-white text-gray-700 hover:bg-gray-50'} rounded-r-md border border-gray-300`}
                >
                  Full Year
                </Button>
              </div>
            ...
            <div className="inline-flex rounded-md shadow-sm">
              {dateOptions.map((option) => (
                <Button
                  ...
                  className={`px-4 py-2 text-sm font-medium ${selectedDateView.value ===
                  option.value
                    ? 'bg-primary text-white'
            ...
            <PageContainerTabs>
              <TabNavigation
                tabs={[
                  ...
                  {
                    id: 'breakdown',
                    label: 'Breakdown by Page Type',
                    disabled: !processedData.pageTypeBreakdown.enabled,
                  },
                ]}
            ...
            <PageContainerBody>
              ...
              {activeKpiTab === 'summary' && (
                <div className="space-y-6">
                  ...
                  <div
                    className="bg-white p-5 rounded-lg border border-lightGray shadow-sm">
                    <h4 className="font-medium text-dark mb-3">Overall KPI
                      Progress</h4>
                    <div className="relative pt-1">
                      <div className="flex mb-2 items-center justify-between">
                        <div>
                      <span
                        className="text-sm font-semibold inline-block py-1 px-2 uppercase rounded-full text-primary bg-blue-100">
                        {processedData.summary.status === 'success'
                          ? 'On Track'
                          : processedData.summary.status === 'warning'
                            ? 'At Risk'
                      ...
                      <span
                        className="text-sm font-semibold inline-block text-primary">
                        {processedData.summary.currentProgress}% of Q2 Goal
                      </span>
                        </div>
                      </div>
                      <div
                        className="overflow-hidden h-4 mb-4 text-xs flex rounded-full bg-blue-100">
                        <div
                          style={{ width: `${processedData.summary.currentProgress}%` }}
                          className={`shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center rounded-full ${processedData.summary.status ===
                          'success'
                            ? 'bg-green-500'
                            : processedData.summary.status === 'warning'
                              ? 'bg-amber-500'
                      ...
                      <div
                        className="flex justify-between text-xs text-mediumGray">
                  ...
                  <div className="grid grid-cols-1 gap-6">
                    {processedData.summary.kpiCards.map((card, index) => (
                      <div key={index}
                           className="bg-white p-5 rounded-lg border border-lightGray shadow-sm relative group">
                        <div className="flex justify-between items-start mb-3">
                          <div className="flex items-center">
                            <h4
                              className="text-base font-medium text-dark">{card.title}</h4>
                            <div className="relative ml-1">
                              <svg xmlns="http://www.w3.org/2000/svg"
                                   className="h-4 w-4 text-mediumGray cursor-help"
                              ...
                              <div
                                className="absolute left-0 bottom-full mb-2 w-48 bg-dark text-white text-xs rounded p-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-10">
                                {card.tooltip}
                              </div>
                            </div>
                          </div>
                          <div
                            className={`flex items-center text-sm ${card.trend ===
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-3">
                          <div className="bg-gray-50 p-3 rounded-lg">
                            <div
                              className="text-xs text-mediumGray mb-1">Current
                            </div>
                          ...
                          <div className="bg-blue-50 p-3 rounded-lg">
                            <div
                              className="text-xs text-mediumGray mb-1">Target
                            </div>
                            <div className="text-2xl font-bold text-dark">
                              {card.unit === '$' &&
                        ...
                        <div className="bg-amber-50 p-3 rounded-lg mb-3">
                          <div className="flex justify-between items-center">
                            ...
                            <span className="font-medium text-dark">{Math.round(
                              (card.target - card.current) / card.target *
                              100)}%</span>
                          </div>
                        </div>
                        ...
                        <div className="w-full bg-lightGray rounded-full h-2.5">
                          <div
                            ...
                            'blue' ? 'bg-blue-500' : card.color === 'green'
                              ? 'bg-green-500'
                              : card.color === 'purple'
                                ? 'bg-purple-500'
                                : 'bg-amber-500'}`}
                            style={{
                              width: `${(card.current / card.target) * 100}%`,
                            }}
                          ></div>
                        </div>
                      </div>
                ...
                <div className="space-y-6">
                  ...
                  <div
                    ...
                    <div className="flex justify-between items-center mb-4">
                      <h4 className="font-medium text-dark">Traffic Projection
                        Visualization</h4>
                      <div className="flex items-center space-x-2">
                        <div className="w-3 h-3 rounded-full bg-blue-500"></div>
                        <span
                          className="text-xs text-mediumGray mr-3">Current</span>
                        <div
                          className="w-3 h-3 rounded-full bg-green-500"></div>
                        <span
                          className="text-xs text-mediumGray mr-3">Target</span>
                        <div
                          className="w-3 h-3 rounded-full bg-amber-500"></div>
                        <span
                          className="text-xs text-mediumGray">Required</span>
                      </div>
                    </div>
                    <div className="h-64">
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          <Line type="monotone" dataKey="current"
                                name="Current Trajectory" stroke="#3b82f6"
                                strokeWidth={2} dot={{ r: 4 }}
                                activeDot={{ r: 6 }}/>
                          <Line type="monotone" dataKey="target" name="KPI Goal"
                                stroke="#22c55e" strokeWidth={2} dot={{ r: 4 }}
                                activeDot={{ r: 6 }}/>
                          <Line type="monotone" dataKey="required"
                                name="Required Output" stroke="#f59e0b"
                    ...
                    <div
                      className="mt-2 text-xs text-mediumGray flex items-center justify-center">
                      <svg xmlns="http://www.w3.org/2000/svg"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div
                      className="bg-white p-5 rounded-lg border border-lightGray shadow-sm">
                      <h4 className="font-medium text-dark mb-3">Current
                        Forecast</h4>
                      <div className="space-y-4">
                        {/* Projected Outcomes */}
                        <div className="p-4 bg-gray-50 rounded-lg">
                          <h5 className="font-medium text-dark mb-2">Projected
                            Outcomes</h5>
                          <ul className="space-y-2 text-sm text-mediumGray">
                            <li className="flex justify-between">
                              <span>Total Forecasted Traffic:</span>
                              <span
                                className="font-medium text-dark">{processedData.forecasting.currentResources.projectedTraffic.toLocaleString()}</span>
                            </li>
                            <li className="flex justify-between">
                              <span>Forecasted Leads:</span>
                              <span
                                className="font-medium text-dark">{processedData.forecasting.currentResources.projectedLeads}</span>
                            </li>
                            <li className="flex justify-between">
                              <span>% of Target:</span>
                              <span
                                className="font-medium text-dark">{processedData.forecasting.currentResources.targetPercentage}%</span>
                            </li>
                          </ul>
                        </div>
                        ...
                        <div>
                          <h5 className="font-medium text-dark mb-2">Current
                            Monthly Deliverables</h5>
                          <div className="grid grid-cols-3 gap-2">
                            <div
                              className="p-3 bg-blue-50 rounded-lg text-center">
                              <div
                                className="text-lg font-bold text-dark">{processedData.forecasting.currentResources.deliverables.briefs}</div>
                              <div className="text-xs text-mediumGray">Briefs
                              </div>
                            </div>
                            <div
                              className="p-3 bg-green-50 rounded-lg text-center">
                              <div
                                className="text-lg font-bold text-dark">{processedData.forecasting.currentResources.deliverables.backlinks}</div>
                              <div
                                className="text-xs text-mediumGray">Backlinks
                              </div>
                            </div>
                            <div
                              className="p-3 bg-purple-50 rounded-lg text-center">
                              <div
                                className="text-lg font-bold text-dark">{processedData.forecasting.currentResources.deliverables.techFixes}</div>
                              <div className="text-xs text-mediumGray">Tech
                                fixes
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="p-3 bg-blue-50 rounded-lg">
                          <p className="text-sm text-mediumGray">At this pace,
                            you're projected to reach these outcomes by <span
                              className="font-medium text-dark">{processedData.forecasting.currentResources.timeline}</span>.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div
                      className="bg-white p-5 rounded-lg border border-lightGray shadow-sm">
                      <h4 className="font-medium text-dark mb-3">Target
                        KPIs</h4>
                      <div className="space-y-4">
                        {/* Target KPI Summary */}
                        <div className="p-4 bg-gray-50 rounded-lg">
                          <h5 className="font-medium text-dark mb-2">Agreed
                            Targets</h5>
                          <ul className="space-y-2 text-sm text-mediumGray">
                            <li className="flex justify-between">
                              <span>Target Traffic:</span>
                              <span
                                className="font-medium text-dark">{processedData.forecasting.agreedTargets.targetTraffic.toLocaleString()}</span>
                            </li>
                            <li className="flex justify-between">
                              <span>Target Leads:</span>
                              <span
                                className="font-medium text-dark">{processedData.forecasting.agreedTargets.targetLeads}</span>
                            </li>
                            <li className="flex justify-between">
                              <span>End Date:</span>
                              <span
                                className="font-medium text-dark">{processedData.forecasting.agreedTargets.endDate}</span>
                            </li>
                          </ul>
                        </div>
                        ...
                        <div className="p-4 bg-amber-50 rounded-lg">
                          <h5 className="font-medium text-dark mb-2">Gap
                            Analysis</h5>
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-mediumGray">Gap to close:</span>
                            <span
                              className="font-medium text-dark">{processedData.forecasting.agreedTargets.gap.percentage}%</span>
                          </div>
                          <div
                            className="flex justify-between items-center mt-1">
                            <span className="text-sm text-mediumGray">Estimated shortfall:</span>
                            <span
                              className="font-medium text-dark">~{processedData.forecasting.agreedTargets.gap.shortfall.toLocaleString()} visits</span>
                          </div>
                        </div>

                        {/* Toggle for Required Adjustments */}
                        <div className="flex justify-between items-center">
                          <h5 className="font-medium text-dark">Required
                            Adjustments</h5>
                          <Button
                            onClick={() => setShowRequiredAdjustments(
                        ...
                        {showRequiredAdjustments && (
                          <div className="space-y-2">
                            <div
                              ...
                              <span
                                className="text-sm text-mediumGray">Deliverables:</span>
                              <span
                                className="font-medium text-dark">+{processedData.forecasting.agreedTargets.requiredAdjustments.content} briefs / month</span>
                            </div>
                            <div
                              className="flex justify-between p-2 bg-gray-50 rounded">
                              <span
                                className="text-sm text-mediumGray">Timeline:</span>
                              <span
                                className="font-medium text-dark">+{processedData.forecasting.agreedTargets.requiredAdjustments.timeline} months</span>
                            </div>
                            <div
                              className="flex justify-between p-2 bg-gray-50 rounded">
                              <span className="text-sm text-mediumGray">Conversion Rate:</span>
                              <span
                                className="font-medium text-dark">↑ from {processedData.forecasting.agreedTargets.requiredAdjustments.conversionRate.current}% → {processedData.forecasting.agreedTargets.requiredAdjustments.conversionRate.required}%</span>
                            </div>
                            <div className="mt-2 text-xs text-mediumGray">
                              <p className="italic">Note: These adjustments are
                                manually entered and can be updated based on
              ...
              {activeKpiTab === 'breakdown' && (
                <div className="space-y-6">
                  ...
                  <div className="flex flex-wrap gap-3 mb-4">
                    <div className="flex items-center space-x-2">
                      <span
                        className="text-sm text-mediumGray">Date Range:</span>
                      <select
                        className="border border-lightGray rounded-md px-2 py-1 text-sm">
                    ...
                    <div className="flex items-center space-x-2">
                      <span
                        className="text-sm text-mediumGray">Funnel Stage:</span>
                      <select
                        className="border border-lightGray rounded-md px-2 py-1 text-sm">
                    ...
                    <div className="flex items-center space-x-2">
                      <span
                        className="text-sm text-mediumGray">Page Type:</span>
                      <select
                        className="border border-lightGray rounded-md px-2 py-1 text-sm">
                  ...
                  <div
                    ...
                    <div
                      className="bg-white p-4 rounded-lg border border-lightGray shadow-sm">
                      <h4 className="text-sm font-medium text-dark mb-2">Blog
                        Pages</h4>
                      <div className="space-y-2">
                        <div className="flex justify-between">
                          <span className="text-xs text-mediumGray">Organic Clicks:</span>
                          <span
                            className="text-sm font-medium text-dark">{processedData.pageTypeBreakdown.overview.blog.clicks.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-xs text-mediumGray">Conversion Rate:</span>
                          <span
                            className="text-sm font-medium text-dark">{processedData.pageTypeBreakdown.overview.blog.conversionRate}%</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-xs text-mediumGray">Leads Captured:</span>
                          <span
                            className="text-sm font-medium text-dark">{processedData.pageTypeBreakdown.overview.blog.leads}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div
                      className="bg-white p-4 rounded-lg border border-lightGray shadow-sm">
                      <h4 className="text-sm font-medium text-dark mb-2">Feature
                        Pages</h4>
                      <div className="space-y-2">
                        <div className="flex justify-between">
                          <span
                            className="text-xs text-mediumGray">Clicks:</span>
                          <span
                            className="text-sm font-medium text-dark">{processedData.pageTypeBreakdown.overview.feature.clicks.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-xs text-mediumGray">Engagement Rate:</span>
                          <span
                            className="text-sm font-medium text-dark">{processedData.pageTypeBreakdown.overview.feature.engagementRate}%</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-xs text-mediumGray">Assisted Conversions:</span>
                          <span
                            className="text-sm font-medium text-dark">{processedData.pageTypeBreakdown.overview.feature.assistedConversions}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div
                      className="bg-white p-4 rounded-lg border border-lightGray shadow-sm">
                      <h4
                        className="text-sm font-medium text-dark mb-2">Solution
                        Pages</h4>
                      <div className="space-y-2">
                        <div className="flex justify-between">
                          <span className="text-xs text-mediumGray">Traffic Volume:</span>
                          <span
                            className="text-sm font-medium text-dark">{processedData.pageTypeBreakdown.overview.solution.traffic.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-xs text-mediumGray">Conversion Rate:</span>
                          <span
                            className="text-sm font-medium text-dark">{processedData.pageTypeBreakdown.overview.solution.conversionRate}%</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-xs text-mediumGray">Avg Time on Page:</span>
                          <span
                            className="text-sm font-medium text-dark">{processedData.pageTypeBreakdown.overview.solution.timeOnPage}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div
                      className="bg-white p-4 rounded-lg border border-lightGray shadow-sm">
                      <h4
                        className="text-sm font-medium text-dark mb-2">High-Intent
                        Pages</h4>
                      <div className="space-y-2">
                        <div className="flex justify-between">
                          <span className="text-xs text-mediumGray">Traffic Volume:</span>
                          <span
                            className="text-sm font-medium text-dark">{processedData.pageTypeBreakdown.overview.highIntent.traffic.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-xs text-mediumGray">Conversion Rate:</span>
                          <span
                            className="text-sm font-medium text-dark">{processedData.pageTypeBreakdown.overview.highIntent.conversionRate}%</span>
                        </div>
                        <div className="flex justify-between">
                          <span
                            className="text-xs text-mediumGray">Leads:</span>
                          <span
                            className="text-sm font-medium text-dark">{processedData.pageTypeBreakdown.overview.highIntent.leads}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div
                    className="bg-white p-5 rounded-lg border border-lightGray shadow-sm">
                    <h4 className="font-medium text-dark mb-3">Funnel Stage
                      Breakdown</h4>
                        ...
                        <div className="h-64">
                          <ResponsiveContainer width="100%" height="100%">
                            ...
                            >
                              ...
                              <Tooltip/>
                              <Legend/>
                              <Bar yAxisId="left" dataKey="traffic"
                                   name="Traffic" fill="#8884d8"/>
                              <Bar yAxisId="right" dataKey="leads" name="Leads"
                                   fill="#82ca9d"/>
                            </BarChart>
                          </ResponsiveContainer>
                        </div>
                          ...
                          <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                              <Pie
                                ...
                                nameKey="stage"
                              >
                                {processedData.pageTypeBreakdown.funnelStages.map(
                                  (_, index) => (
                                    <Cell key={`cell-${index}`}
                    ...
                    <div className="overflow-x-auto">
                      <table className="min-w-full divide-y divide-lightGray">
                        ...
                        <tr>
                          <th
                            className="px-4 py-3 text-left text-xs font-medium text-mediumGray uppercase tracking-wider">Stage
                          </th>
                          <th
                            className="px-4 py-3 text-left text-xs font-medium text-mediumGray uppercase tracking-wider">Pages
                          </th>
                          <th
                            className="px-4 py-3 text-left text-xs font-medium text-mediumGray uppercase tracking-wider">Traffic
                          </th>
                          <th
                            className="px-4 py-3 text-left text-xs font-medium text-mediumGray uppercase tracking-wider">Conv.
                            Rate
                          </th>
                          <th
                            className="px-4 py-3 text-left text-xs font-medium text-mediumGray uppercase tracking-wider">Leads
                          </th>
                        </tr>
                        </thead>
                        ...
                        {processedData.pageTypeBreakdown.funnelStages.map(
                          (stage, index) => (
                            <tr key={index}>
                              <td
                                className="px-4 py-3 whitespace-nowrap text-sm font-medium text-dark">{stage.stage}</td>
                              <td
                                className="px-4 py-3 whitespace-nowrap text-sm text-mediumGray">{stage.pages}</td>
                              <td
                                className="px-4 py-3 whitespace-nowrap text-sm text-mediumGray">{stage.traffic.toLocaleString()}</td>
                              <td
                                className="px-4 py-3 whitespace-nowrap text-sm text-mediumGray">{stage.conversionRate}%
                              </td>
                              <td
                                className="px-4 py-3 whitespace-nowrap text-sm text-mediumGray">{stage.leads}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div
                      className="bg-white p-5 rounded-lg border border-lightGray shadow-sm">
                      <h4 className="font-medium text-dark mb-3">Top
                        Performers</h4>
                      <div className="space-y-3">
                        {processedData.pageTypeBreakdown.performers.top.map(
                          (performer, index) => (
                            <div key={index}
                              ...
                              <div className="flex justify-between items-start">
                                <div>
                                  <p
                                    ...
                                    style={{ maxWidth: '200px' }}>{performer.url}</p>
                                  <div className="flex items-center mt-1">
                                    <span
                                      className="text-xs text-mediumGray mr-2">{performer.metric}:</span>
                                    <span
                                      className="text-xs font-medium text-dark">{performer.value}</span>
                                    <span
                                      className="ml-2 text-xs text-green-500">↑ {performer.change}%</span>
                                  </div>
                                </div>
                                <span
                                  className="px-2 py-1 text-xs bg-green-100 text-green-800 rounded">{performer.tag}</span>
                              </div>
                            </div>
                          ))}
                      </div>
                    ...
                    <div
                      className="bg-white p-5 rounded-lg border border-lightGray shadow-sm">
                      <h4 className="font-medium text-dark mb-3">Underperforming
                        Pages</h4>
                      <div className="space-y-3">
                        {processedData.pageTypeBreakdown.performers.bottom.map(
                          (performer, index) => (
                            <div key={index}
                                ...
                                <div>
                                  ...
                                  <div className="flex items-center mt-1">
                                    ...
                                    <span
                                      className="text-xs font-medium text-dark">{performer.value}</span>
                                    <span
                                      className="ml-2 text-xs text-red-500">↓ {Math.abs(
                                      performer.change)}%</span>
                                  </div>
                                </div>
                                <span
                                  className="px-2 py-1 text-xs bg-red-100 text-red-800 rounded">{performer.tag}</span>
                              </div>
                            </div>
                          ))}
                      </div>
                  ...
                  <div
                    ...
                    <h4 className="font-medium text-dark mb-3">Bottleneck
                      Insights</h4>
                    <ul className="space-y-2">
                      {processedData.pageTypeBreakdown.bottlenecks.map(
                        (bottleneck, index) => (
                          <li key={index} className="flex items-start">
                            <span className="text-blue-500 mr-2">•</span>
                            <span
                              className="text-sm text-mediumGray">{bottleneck}</span>
                          </li>
                        ))}
                    </ul>
                  </div>
