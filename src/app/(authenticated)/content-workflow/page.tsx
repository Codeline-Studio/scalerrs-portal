...
function ContentItemCard ({
  ...
  return (
    <div
      className="bg-white p-4 rounded-scalerrs border border-lightGray shadow-sm">
...
function ContentDetailModal ({
  ...
  return (
    <div
      ...
      <div
        className="bg-white p-6 rounded-scalerrs shadow-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          ...
          <div>
            ...
            <div className="font-medium text-dark">{new Date(
              item.dueDate).toLocaleDateString()}</div>
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <div
            className="bg-lightGray p-3 rounded-scalerrs text-dark">{item.notes}</div>
        </div>
        ...
        <div>
          ...
          <div className="relative">
            <div
              className="absolute left-3 top-0 bottom-0 w-0.5 bg-lightGray"></div>
            <div className="space-y-4 ml-6">
              {item.history.map((event: any, index: number) => (
                <div key={index} className="relative">
                  <div
                    className="absolute -left-6 mt-1 w-4 h-4 rounded-full bg-white border-2 border-primary"></div>
                  <div>
                    ...
                    <div className="text-sm text-dark mt-1">
                      Moved to {stages.find(
                      s => s.id === event.stage)?.name} by {event.user}
                    </div>
                  </div>
                </div>
              ))}
...
export default function ContentWorkflow () {
  ...
  const [detailModal, setDetailModal] = useState(
    { isOpen: false, itemId: null as number | null })
  ...
  return (
    <>
      ...
      <div
        className="bg-white rounded-scalerrs shadow-sm border border-lightGray mb-8">
        <div className="p-4 border-b border-lightGray overflow-x-auto">
          <div className="flex flex-wrap gap-2">
            <Button
              className={`px-3 py-1 text-xs font-medium rounded-full ${activeFilter ===
              'all'
                ? 'bg-primary text-white'
                : 'bg-lightGray text-mediumGray hover:bg-gray-300'}`}
            ...
            {stages.map(stage => (
              <Button
                ...
                className={`px-3 py-1 text-xs font-medium rounded-full ${
                  activeFilter === stage.id ? stage.color.replace('bg-', 'bg-').
                    replace('/10', '') + ' text-white' : stage.color
                }`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredItems.length > 0 ? (
              ...
              <div className="col-span-3 text-center py-8">
                <p className="text-mediumGray">No content items found in this
                  stage.</p>
              </div>
            )}
          </div>
      ...
      <div className="bg-lightGray p-4 rounded-scalerrs">
        <p className="text-sm text-mediumGray">
          <strong>Note:</strong> Content workflow is synchronized with our
          content management system. Changes made here will be reflected in the
          main system within 5 minutes.
        </p>
      </div>
