...
export default function AirtableDemoPage () {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const hasAirtableIssues = localStorage.getItem(
        'airtable-connection-issues') === 'true'
    ...
    const getComments = async () => {
      try {
        ...
        if (data && Array.isArray(data) && data.length === 0) {
          ...
          console.log(
            '2. Task ID format mismatch between Airtable and our app')
        ...
        if (data && Array.isArray(data)) {
          if (data.length === 0) {
            ...
            console.log(
              `Found ${data.length} comments for task ${selectedTask.id}`)
          ...
          const mockTaskComments = mockComments.filter(
            c => c.Task.includes(selectedTask.id))
        ...
        const mockTaskComments = mockComments.filter(
          c => c.Task.includes(selectedTask.id))
        console.log('Using mock comments as fallback after error:',
          mockTaskComments)
        setComments(mockTaskComments)
        setLoadingComments(false)
      }
  ...
  const addComment = async () => {
    ...
    try {
      const newCommentData = await addTaskComment(selectedTask.id, user.id,
        newComment)
      ...
      <div className="container mx-auto py-6 px-4">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center">
            ...
            {usingMockData && (
              <span
                className="ml-3 px-2 py-1 text-xs bg-yellow-100 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-100 rounded-full">
                Using Mock Data
              </span>
            )}
          </div>
        ...
        {debugMode && (
          <div
            className="bg-white dark:bg-darkGray border border-lightGray dark:border-gray-700 p-4 rounded-scalerrs shadow-sm mb-4">
            <h2 className="text-lg font-semibold mb-2">Debug Information</h2>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <h3 className="text-md font-medium mb-1">Tasks
                  ({tasks.length})</h3>
                <div
                  className="max-h-40 overflow-y-auto bg-gray-50 dark:bg-gray-900 p-2 rounded-md border border-lightGray dark:border-gray-700">
              ...
              <div className="md:col-span-2">
                <h3 className="text-md font-medium mb-1">Comments
                  ({comments.length})</h3>
                <div
                  className="max-h-40 overflow-y-auto bg-gray-50 dark:bg-gray-900 p-2 rounded-md border border-lightGray dark:border-gray-700">
                  <pre className="text-xs">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Tasks List */}
            <div
              className="bg-white dark:bg-darkGray rounded-scalerrs shadow p-4">
                  ...
                  {tasks.map(task => (
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <span className="font-medium">{task.Name ||
                          task.Title}</span>
            ...
            {/* Task Details */}
            <div
              className="bg-white dark:bg-darkGray rounded-scalerrs shadow p-4 md:col-span-2">
              {selectedTask ? (
                <div>
                  <h2
                    className="text-xl font-semibold mb-4">{selectedTask.Name ||
                    selectedTask.Title}</h2>

                  {selectedTask.Description && (
                    <p
                      className="text-mediumGray mb-4">{selectedTask.Description}</p>
                  )}
                  ...
                  <div className="mb-6">
                    ...
                    <div className="flex space-x-2">
                      <Button
                        variant={selectedTask.Status === 'To Do'
                          ? 'primary'
                          : 'outline'}
                        size="sm"
                      ...
                      <Button
                        variant={selectedTask.Status === 'In Progress'
                          ? 'primary'
                          : 'outline'}
                        size="sm"
                        onClick={() => updateTaskStatus(selectedTask.id,
                      ...
                      {loadingComments ? (
                        ...
                        <ul className="space-y-3">
                          {comments.map(comment => (
                            <li key={comment.id}
                                className="p-3 rounded-md border border-lightGray bg-primary/5 dark:border-gray-700 dark:bg-primary/10">
                              {/* Display the title if available */}
                              {comment.Title && (
                                <p
                                  className="text-sm font-medium mb-1">{comment.Title}</p>
                              )}
                              ...
                              <p className="text-xs text-mediumGray mt-1">
                                {(() => {
                                  ...
                                  const dateValue = comment.CreatedAt ||
                              ...
                              {comment.User && (
                                <p className="text-xs text-primary mt-1">
                                  ...
                                  if (typeof comment.User ===
                                    'string') return comment.User
                                  return 'Unknown user'
                                })()}
                                </p>
                              )}
                              ...
                              {debugMode && (
                                <details
                                  className="mt-2 text-xs text-mediumGray">
