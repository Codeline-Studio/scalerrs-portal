...
import MonthlyProjectionsBoard
  from '@/components/task-boards/monthly-projections-board'
      ...
      comments: [
        {
          id: 1,
          author: 'Acex Romero',
          text: 'Found 15 broken links so far',
          timestamp: 'Mar 12',
        },
      ],
      ...
      comments: [
        {
          id: 1,
          author: 'Taylor Green',
          text: 'All issues fixed and verified',
          timestamp: 'Mar 28',
        },
      ],
      ...
      comments: [
        {
          id: 1,
          author: 'Acex Romero',
          text: 'Testing 3 different button styles',
          timestamp: 'Mar 15',
        },
      ],
    } as ActiveTask,
      ...
      comments: [
        {
          id: 1,
          author: 'Taylor Green',
          text: 'Reduced form fields from 8 to 5',
          timestamp: 'Mar 20',
        },
        {
          id: 2,
          author: 'Acex Romero',
          text: 'Conversion rate improved by 12%',
          timestamp: 'Apr 01',
        },
        {
          id: 3,
          author: 'Taylor Green',
          text: 'Added inline validation',
          timestamp: 'Apr 02',
        },
        {
          id: 4,
      ...
      comments: [
        {
          id: 1,
          author: 'Schonn Force',
          text: 'A/B test complete - 8% improvement',
          timestamp: 'Feb 25',
        },
        {
          id: 2,
      ...
      comments: [
        {
          id: 1,
          author: 'Acex Romero',
          text: 'Found 25 high-opportunity keywords',
          timestamp: 'Mar 10',
        },
      ],
  ...
  return (
    <div className="mb-2 last:mb-0">
      <div className="flex items-start">
        <div
          className="bg-lightGray rounded-full w-6 h-6 flex items-center justify-center mr-2 mt-0.5">
          <span className="text-xs font-medium">{comment.author.charAt(
            0)}</span>
        </div>
        <div className="flex-1">
          <div className="flex items-baseline">
            <span
              className="text-xs font-medium text-dark">{comment.author}</span>
            <span
              className="text-xs text-mediumGray ml-2">{comment.timestamp}</span>
          </div>
    ...
    if (isExpanded) {
      const fetchTaskComments = async () => {
        try {
          ...
          const mappedComments: Comment[] = commentsData.map(
            (comment: any) => ({
              id: comment.id,
          ...
          setTaskComments(
            mappedComments.length > 0 ? mappedComments : comments)
        } catch (err) {
  ...
  return (
    <div className="mt-3">
      ...
      >
        ...
        {taskComments.length} {taskComments.length === 1
        ? 'Comment'
        : 'Comments'}
      </Button>
          ...
          <div className="bg-lightGray/30 p-3 rounded-md mb-2">
            {loading ? (
              <div className="flex justify-center items-center py-4">
                <div
                  className="animate-spin rounded-full h-6 w-6 border-t-2 border-b-2 border-primary"></div>
              </div>
...
function TaskCard ({
  ...
  return (
    <div
      className="card bg-white p-4 rounded-scalerrs border border-lightGray shadow-sm"
      style={{ color: '#353233' }}>
      <div className="flex justify-between items-start mb-3">
        <h3
          className="text-md font-medium text-text-light dark:text-text-dark">{task.task}</h3>
      ...
      <div className="flex items-center space-x-3 mb-3">
        <div>
          <span
            className="text-xs text-mediumGray dark:text-gray-300 block mb-1">Impact</span>
          <ImpactBadge impact={task.impact}/>
        </div>
        <div>
          <span
            className="text-xs text-mediumGray dark:text-gray-300 block mb-1">Effort</span>
      ...
      {task.notes && (
        <div className="mb-3">
          <div
            className="text-xs text-mediumGray dark:text-gray-300 mb-1">Notes
          </div>
          <p
            className="text-sm text-text-light dark:text-text-dark bg-lightGray/30 dark:bg-darkGray/30 p-2 rounded">{task.notes}</p>
        </div>
      )}
...
function TaskTable ({
  ...
  const [expandedTaskId, setExpandedTaskId] = useState<number | string | null>(
    null)
  ...
  const sortedTasks = [...tasks].sort((a, b) => {
    ...
    const priorityComparison = priorityOrder[a.priority] -
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tr className="bg-lightGray/50 border-b border-lightGray">
            <th
              className="px-4 py-3 text-left text-xs font-medium text-mediumGray dark:text-gray-300 uppercase tracking-wider">Task
            </th>
            <th
              className="px-4 py-3 text-left text-xs font-medium text-mediumGray dark:text-gray-300 uppercase tracking-wider">Status
            </th>
            <th
              className="px-4 py-3 text-left text-xs font-medium text-mediumGray dark:text-gray-300 uppercase tracking-wider">Assigned
              to
            </th>
            <th
              className="px-4 py-3 text-left text-xs font-medium text-mediumGray dark:text-gray-300 uppercase tracking-wider">Date
              Logged
            </th>
            <th
              className="px-4 py-3 text-left text-xs font-medium text-mediumGray dark:text-gray-300 uppercase tracking-wider">Priority
            </th>
            <th
              className="px-4 py-3 text-left text-xs font-medium text-mediumGray dark:text-gray-300 uppercase tracking-wider">Impact
            </th>
            <th
              className="px-4 py-3 text-left text-xs font-medium text-mediumGray dark:text-gray-300 uppercase tracking-wider">Effort
            </th>
            <th
              className="px-4 py-3 text-left text-xs font-medium text-mediumGray dark:text-gray-300 uppercase tracking-wider">Comments
            </th>
            <th
              className="px-4 py-3 text-left text-xs font-medium text-mediumGray dark:text-gray-300 uppercase tracking-wider">Actions
            </th>
          </tr>
          </thead>
          ...
          {sortedTasks.map((task) => (
            <React.Fragment key={task.id}>
              <tr className="hover:bg-lightGray/20 cursor-pointer"
                  onClick={() => setExpandedTaskId(
                    expandedTaskId === task.id ? null : task.id)}>
                <td
                  className="px-4 py-3 text-sm text-text-light dark:text-text-dark">{task.task}</td>
                <td className="px-4 py-3 text-sm">
                  <StatusBadge status={task.status}/>
                </td>
                <td
                  className="px-4 py-3 text-sm text-text-light dark:text-text-dark">{task.assignedTo}</td>
                <td
                  className="px-4 py-3 text-sm text-text-light dark:text-text-dark">{task.dateLogged}</td>
                <td className="px-4 py-3 text-sm">
                  <PriorityBadge priority={task.priority}/>
                </td>
                <td className="px-4 py-3 text-sm">
                ...
                <td className="px-4 py-3 text-sm">
                    <span
                      className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-lightGray dark:bg-darkGray text-mediumGray dark:text-gray-300">
                  ...
                  <td colSpan={9} className="px-4 py-3 bg-lightGray/10">
                    <div className="grid grid-cols-2 gap-4">
                      {task.notes && (
                        <div>
                          <h4
                            className="text-sm font-medium text-dark mb-1">Notes</h4>
                          <p
                            className="text-sm text-dark bg-white p-2 rounded border border-lightGray">{task.notes}</p>
                        </div>
                      )}
                      <div>
                        <h4
                          className="text-sm font-medium text-dark mb-1">Comments</h4>
                        <div
                          className="bg-white p-2 rounded border border-lightGray">
                          {task.comments.length > 0 ? (
                            task.comments.map((comment) => (
                              <CommentItem key={comment.id} comment={comment}/>
                            ))
                          ) : (
                            <p className="text-sm text-mediumGray">No comments
                              yet</p>
                          )}
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (taskData.task.trim() && taskData.assignedTo.trim()) {
      ...
      const newTask: Task = {
        ...
        id: Date.now(),
        status: 'Not Started',
        dateLogged: new Date().toLocaleDateString('en-US',
          { month: 'short', day: 'numeric' }),
        comments: [],
    ...
    <div
      ...
      <div className="bg-white p-6 rounded-scalerrs shadow-lg max-w-xl w-full">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-medium text-dark">Add New Task
            to {boardType} Board</h3>
          <Button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-mediumGray mb-1">Task
                Name</label>
              <input
                type="text"
                ...
                onChange={(e) => setTaskData(
                  { ...taskData, task: e.target.value })}
                required
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-mediumGray mb-1">Assigned
                To</label>
              <input
                type="text"
                className="w-full border border-lightGray rounded-scalerrs p-2 focus:outline-none focus:ring-2 focus:ring-primary"
            ...
            <div>
              <label
                className="block text-sm font-medium text-mediumGray mb-1">Priority</label>
              <select
                className="w-full border border-lightGray rounded-scalerrs p-2 focus:outline-none focus:ring-2 focus:ring-primary"
                value={taskData.priority}
                onChange={(e) => setTaskData(
                  { ...taskData, priority: e.target.value as TaskPriority })}
              >
            ...
            <div>
              <label className="block text-sm font-medium text-mediumGray mb-1">Impact
                (1-5)</label>
              <select
                className="w-full border border-lightGray rounded-scalerrs p-2 focus:outline-none focus:ring-2 focus:ring-primary"
                value={taskData.impact}
                onChange={(e) => setTaskData(
                  { ...taskData, impact: parseInt(e.target.value) })}
              >
            ...
            <div>
              <label
                className="block text-sm font-medium text-mediumGray mb-1">Effort</label>
              <select
                className="w-full border border-lightGray rounded-scalerrs p-2 focus:outline-none focus:ring-2 focus:ring-primary"
                value={taskData.effort}
                onChange={(e) => setTaskData(
                  { ...taskData, effort: e.target.value as TaskEffort })}
              >
            ...
            <div className="md:col-span-2">
              <label
                className="block text-sm font-medium text-mediumGray mb-1">Notes</label>
              <textarea
                ...
                onChange={(e) => setTaskData(
                  { ...taskData, notes: e.target.value })}
              />
            </div>

            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-mediumGray mb-1">Reference
                Links</label>
              <textarea
                ...
                onChange={(e) => setTaskData(
                  { ...taskData, referenceLinks: e.target.value })}
              />
            </div>
          </div>
...
export default function TaskBoards () {
  ...
  const showStrategyTab = boards.strategyAdHoc && boards.strategyAdHoc.length >
    0

  const handleStatusChange = async (
    id: number | string, newStatus: TaskStatus) => {
    try {
      ...
      setBoards(prev => {
        ...
        const taskIndex = newBoards[activeBoard as keyof typeof boards].findIndex(
          task =>
    ...
    <DashboardLayout>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-mediumGray">Collaborate on action items with clear
            priorities and ownership</p>
        </div>
        ...
        <PageContainerTabs>
          <TabNavigation
            tabs={[
              ...
              {
                id: 'strategyAdHoc',
                label: 'Strategy / Ad Hoc',
                disabled: !showStrategyTab,
              },
      ...
      <div className="bg-lightGray p-4 rounded-scalerrs mt-8">
        <p className="text-sm text-mediumGray">
          <strong>Note:</strong> Tasks are synchronized with our project
          management system. Changes made here will be reflected in the main
      ...
      <AddTaskModal
        ...
        onAdd={handleAddTask}
        boardType={activeBoard === 'technicalSEO'
          ? 'Technical SEO'
