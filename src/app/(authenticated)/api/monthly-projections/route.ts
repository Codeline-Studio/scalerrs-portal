...
import { getMonthlyProjections } from '@/lib/airtable'
...
export async function GET () {
  try {
    ...
    console.log(
      `API route: Found ${monthlyProjections.length} monthly projections`)
    return NextResponse.json({ monthlyProjections })
  } catch (error) {
    console.error('Error fetching monthly projections:', error)
    console.log(
      'API route: Error fetching monthly projections, using mock data')
    return NextResponse.json({ monthlyProjections: mockMonthlyProjections })
  }
}
