...
export default function DeliverablePage () {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        try {
          ...
          console.log('Briefs fetched successfully:', briefsData.length,
            'records')
        ...
        try {
          ...
          console.log('Articles fetched successfully:', articlesData.length,
            'records')
          ...
          errorMessages.push(
            `Articles: ${articlesErr.message || 'Unknown error'}`)
          hasErrors = true
        ...
        try {
          ...
          console.log('Backlinks fetched successfully:', backlinksData.length,
            'records')
          ...
          errorMessages.push(
            `Backlinks: ${backlinksErr.message || 'Unknown error'}`)
          hasErrors = true
        ...
        try {
          ...
          console.log('URL performance data fetched successfully:',
            urlPerformanceData.length, 'records')
          logData(urlPerformanceData, 'URL Performance')
          setUrlPerformance(urlPerformanceData)
        } catch (urlPerformanceErr: any) {
          console.error('Error fetching URL performance data:',
            urlPerformanceErr)
          errorMessages.push(
            `URL Performance: ${urlPerformanceErr.message || 'Unknown error'}`)
          hasErrors = true
        ...
        setError(
          `An error occurred while fetching deliverables data: ${err.message ||
          'Unknown error'}`)
  ...
  useEffect(() => {
    ...
    if (articles.length > 0) {
      const filtered = articles.filter(
        article => article.Month === selectedMonth)
      setFilteredArticles(filtered)
    }

    if (backlinks.length > 0) {
      // Start with month filter
      let filtered = backlinks.filter(
        backlink => backlink.Month === selectedMonth)

      // Apply status filter if not 'all'
      if (statusFilter !== 'all') {
        filtered = filtered.filter(
          backlink => backlink.Status === statusFilter)
      }
      ...
      if (drFilter !== 'all') {
        if (drFilter === '50+') {
          filtered = filtered.filter(
            backlink => (backlink['Domain Authority/Rating'] ||
              backlink.DomainRating || 0) >= 50)
        } else if (drFilter === '60+') {
          filtered = filtered.filter(
            backlink => (backlink['Domain Authority/Rating'] ||
              backlink.DomainRating || 0) >= 60)
        } else if (drFilter === '70+') {
          filtered = filtered.filter(
            backlink => (backlink['Domain Authority/Rating'] ||
              backlink.DomainRating || 0) >= 70)
        }
      }
  ...
  const handleBriefStatusChange = async (
    ...
    try {
      ...
      // Check if we should use mock data
      const isBrowser = typeof window !== 'undefined'
      const useMockData = isBrowser && localStorage.getItem('use-mock-data') ===
        'true'
      ...
      if (useMockData) {
        ...
        console.log(
          `Brief ${briefId} status updated to ${newStatus} (mock data)`)
        return
      }
      ...
      setError(
        'Could not update brief status in Airtable. Changes are saved locally only.')
    }
  }
  ...
  const handleArticleStatusChange = async (
    ...
    try {
      ...
      if (useMockData) {
        ...
        console.log(
          `Article ${articleId} status updated to ${newStatus} (mock data)`)
        return
      }
      ...
      // Show an error message to the user
      setError(
        'Could not update article status in Airtable. Changes are saved locally only.')
    }
  }
    ...
    <main className="flex flex-1 flex-col gap-6 p-4 md:gap-8 md:p-6">
      ...
      {loading ? (
        <div className="flex justify-center items-center h-64">
          ...
          <span
            className="ml-3 text-mediumGray">Loading deliverables data...</span>
        </div>
      ) : (
