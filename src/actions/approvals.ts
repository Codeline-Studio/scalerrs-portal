import { filter, length } from "remeda";
...
export const getApprovalItems = async (): Promise<ApprovalItems> => {
...
export const getPendingCounts = (items: ApprovalItems) => {
  return {
    keywords: length(
      filter(items.keywords, (item) =>
        ["awaiting_approval", "resubmitted"].includes(item.status)
      )
    ),
    briefs: length(
      filter(items.briefs, (item) =>
        ["awaiting_approval", "resubmitted"].includes(item.status)
      )
    ),
    articles: length(
      filter(items.articles, (item) =>
        ["awaiting_approval", "resubmitted"].includes(item.status)
      )
    ),
    backlinks: length(
      filter(items.backlinks, (item) =>
        ["awaiting_approval", "resubmitted"].includes(item.status)
      )
    ),
    quickwins: length(
      filter(items.quickwins, (item) =>
        ["awaiting_approval", "resubmitted"].includes(item.status)
      )
    ),
  };
